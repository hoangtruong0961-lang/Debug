...
import { X, Search, Upload, Trash2, Play, Users, Link2, Filter, AlertCircle, FileText, Settings, BookOpen, RefreshCw, Lock } from 'lucide-react';
...
export const CharacterLibraryScreen: React.FC<CharacterLibraryScreenProps> = ({ onClose, onGameStart, playerProfile = DEFAULT_PLAYER }) => {
  ...
  const handleStartGame = async (char: StoredCharacter, alternateGreetingIndex?: number) => {
     ...
     const dataBlock = (clonedChar.spec === 'chara_card_v2') ? clonedChar.rawData.data : clonedChar.rawData;
      ...
      <AnimatePresence>
        {!isUnlocked && (
          <motion.div 
            ...
            className="absolute inset-0 z-[100] flex items-center justify-center bg-zinc-950/90 backdrop-blur-md"
          >
            {/* Wires container - animate out by sliding away */}
            <motion.div 
              exit={{ scale: 1.5, opacity: 0 }}
              transition={{ duration: 0.8 }}
              className="absolute inset-0 pointer-events-none flex items-center justify-center overflow-hidden opacity-80 z-0"
            >
              <div className="absolute w-[200%] h-[4px] bg-gradient-to-r from-zinc-900 via-zinc-600 to-zinc-900 rotate-[15deg] shadow-[0_0_20px_black] border-y border-zinc-800" />
              <div className="absolute w-[200%] h-[4px] bg-gradient-to-r from-zinc-900 via-zinc-600 to-zinc-900 -rotate-[15deg] shadow-[0_0_20px_black] border-y border-zinc-800" />
              <div className="absolute w-[200%] h-[2px] bg-gradient-to-r from-zinc-900 via-zinc-500 to-zinc-900 rotate-[40deg] shadow-[0_0_15px_black] border-y border-zinc-800 opacity-60" />
              <div className="absolute w-[200%] h-[2px] bg-gradient-to-r from-zinc-900 via-zinc-500 to-zinc-900 -rotate-[40deg] shadow-[0_0_15px_black] border-y border-zinc-800 opacity-60" />
            </motion.div>
            ...
            <AnimatePresence mode="wait">
              {!showPasswordPrompt ? (
                <motion.button 
                  ...
                  className="relative z-10 w-32 h-32 bg-gradient-to-b from-zinc-800 to-zinc-950 rounded-2xl flex flex-col items-center justify-center shadow-[0_0_50px_rgba(0,0,0,1)] border-[3px] border-zinc-600 group cursor-pointer"
                >
                  {/* Lock shackle */}
                  <div className="absolute -top-10 w-16 h-12 border-8 border-zinc-400 rounded-t-full border-b-0 group-hover:-translate-y-2 transition-transform duration-300 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)]" />
                  <Lock size={40} className="text-zinc-400 mt-2" />
                  <span className="text-zinc-500 text-[10px] font-bold mt-3 uppercase tracking-widest">Locked</span>
                </motion.button>
              ) : (
                <motion.div 
                  key="auth-prompt"
                  ...
                  className="relative z-10 bg-zinc-900/95 backdrop-blur-xl p-8 rounded-3xl border border-zinc-700 shadow-2xl w-80 flex flex-col items-center"
                >
                  <Lock size={28} className="text-zinc-500 mb-3" />
                  <h3 className="text-zinc-200 font-bold mb-6 text-sm tracking-[0.15em] uppercase">Authentication</h3>
                  <input 
                    type="password"
                    value={passwordInput}
                    onChange={(e) => {
                    ...
                    className={`w-full bg-zinc-950 border-2 ${error ? 'border-red-500/50 focus:border-red-500 text-red-100' : 'border-zinc-800 focus:border-indigo-500 text-zinc-100'} text-center text-lg tracking-[0.2em] rounded-xl px-4 py-3 focus:outline-none transition-colors mb-2 font-mono`}
                  />
                  <div className="h-6 mb-4 w-full text-center">
                    <AnimatePresence>
                      {error && (
                        <motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-red-400 text-[10px] font-bold uppercase tracking-widest">
                          Access Denied
                        </motion.p>
                      )}
                    </AnimatePresence>
                  ...
                  <div className="flex gap-4 w-full">
                    <Button variant="ghost" className="flex-1 text-zinc-400 text-sm" onClick={() => { setShowPasswordPrompt(false); setPasswordInput(''); setError(false); }}>Cancel</Button>
                    <Button variant="primary" className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-sm" onClick={handleUnlock}>Unlock</Button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
