...
export function runRegexScript(
    ...
    try {
        return rawString.replace(re, (...args) => {
            ...
            // If the result is a full HTML document or code block, encode it strictly to base64 here
            // to bypass the markdown pipeline and prevent template literals (`) from being corrupted
            if (/^```(?:html|javascript|js|jsx|react|ts|typescript)\b|\b<!DOCTYPE\s+html>/i.test(result)) {
                let code = result;
...
export function getRegexedString(
    ...
    applicableScripts.forEach(script => {
